...
const ServicesCard = ({ description, icon, title, isPopular }: ServicesType) => {
  return (
    <Card className={`border-0 text-center ${isPopular && 'bg-primary bg-shape'}`}>
      <CardBody>
        <div className="mb-3">
          <IconifyIcon icon={icon} className={`fs-1  ${isPopular ? 'text-white' : 'text-primary'}`} />
        </div>
        <h6 className={`${isPopular && 'text-white'}`}>{title}</h6>
        <p className={`mt-2  ${isPopular ? 'text-white-50' : 'text-muted'}`}>{description}</p>
        <Link href="" className={`${isPopular ? 'text-white' : 'text-primary'}`}>
          View More
        </Link>
      </CardBody>
    </Card>
